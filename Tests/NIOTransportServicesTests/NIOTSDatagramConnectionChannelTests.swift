...
class NIOTSDatagramConnectionChannelTests: XCTestCase {
    ...
    func testConnectingToSocketAddressTraversesPipeline() throws {
        ...
        let connectBootstrap = NIOTSDatagramConnectionBootstrap(group: self.group)
    ...
    func testZeroLengthWritesHaveSatisfiedPromises() throws {
        ...
        let connection = try NIOTSDatagramConnectionBootstrap(group: self.group).connect(to: listener.localAddress!).wait()
        defer {
            XCTAssertNoThrow(try connection.close().wait())
        }
    ...
    func testSettingUDPOptionsWholesale() throws {
    ...
    func testSettingGettingReuseaddr() throws {
        let listener = try NIOTSDatagramListenerBootstrap(group: self.group).bind(host: "localhost", port: 0).wait()
        defer {
            XCTAssertNoThrow(try listener.close().wait())
        }
        let connection = try NIOTSDatagramConnectionBootstrap(group: self.group)
    ...
    func testErrorsInChannelSetupAreFine() throws {
        ...
        let connectFuture = NIOTSDatagramConnectionBootstrap(group: self.group)
    ...
    func testCanSafelyInvokeActiveFromMultipleThreads() throws {
        ...
        let channel = try NIOTSDatagramConnectionBootstrap(group: self.group)
    ...
    func testConnectingChannelsOnShutdownEventLoopsFails() throws {
        ...
        let bootstrap = NIOTSDatagramConnectionBootstrap(group: temporaryGroup)
